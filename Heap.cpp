...
vector<double> running_median(const vector<int>& inputs) {
  ...
  for (auto v : inputs) {
    ...
    // cout << "   .. min - top[" << min_heap.top() << "] size[" << min_heap.size() << "], max top[" << max_heap.top()
...
bool test_merge_sorted_array() {
  ...
  res &= merged.size() == accumulate(vecs.begin(), vecs.end(), 0, [](auto agg, auto vec) { return agg + vec.size(); });
