...
namespace Impl {
    ...
    class QueueWithMax
    {
        ...
        void check_empty() const { if(first_ == nullptr) throw runtime_error("empty queue"); }
    public:
        T peek() const {
            check_empty();
            return first_->data;
        ...
        void push(const T& val) {
            ...
            while(!max_deque_.empty() && max_deque_.back() < val)
                max_deque_.pop_back();
        ...
        int size() const { return size_; }
        T max() const { check_empty(); return max_deque_.front(); }
    };
}
...
bool test_stack() {
    ...
    for_each(inputs.begin(), inputs.end(), [&](int x) { int y = st.pop(); cout << "popped -> " << y << endl; });
...
bool test_queue() {
    ...
    for_each(inputs.begin(), inputs.end(), [&](int x) { int y = qu.pop(); cout << "popped -> " << y << endl; });
